...
namespace Pintushi\Bundle\OrganizationBundle\Entity\Manager;
...
class BusinessUnitManager
{
    ...
    {
        return $this->businessUnitRepository->getBusinessUnitsTree($entity, $organizationId);
    }
    ...
    {
        return $this->businessUnitRepository->getBusinessUnitIds($organizationId);
    }
    ...
    {
        return $this->businessUnitRepository->findOneBy($criteria, $orderBy);
    }
    ...
    public function canBusinessUnitBeSetAsOwner(
        ...
        if (AccessLevel::SYSTEM_LEVEL === $accessLevel) {
            $businessUnits = $this->getBusinessUnitIds($organization->getId());
    ...
    {
        $qb = $this->businessUnitRepository->createQueryBuilder('bu');
        $result = $qb
            ->join('bu.users', 'u')
